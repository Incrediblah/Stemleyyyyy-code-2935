...
import frc.robot.Commands.conveyorClearCmd;
...
//intake and conveyor sensor control
 ...
 //shooter subwoofer control
   ...
   //new shooter control stuff with toggle
    /*Command shooterOnCommand = new ShooterCmd(
            ShooterSubsystem, 
            ShooterConstants.subwooferTopVelocity, 
            ShooterConstants.subwooferBottomVelocity
        );

        Command shooterOffCommand = new ShooterCmd(
            ShooterSubsystem, 
            0, 
            0
        );

     shooterToggle.toggleOnTrue(shooterOnCommand.until(() -> !shooterToggle.getAsBoolean()))
    .onFalse(shooterOffCommand);/* */
 ...
 //zero robot control
  ...
  public Command getAutonomousCommand() {
    ...
    .andThen(new ParallelDeadlineGroup(firstCommand,new intakeCmd(IntakeSubsystem, IntakeConstants.intakeVelocity), new conveyorSensorCmd(ConveyorSubsystem,ConveyorConstants.kConveyorVelocity)))
    .andThen(Commands.waitSeconds(0.15))
    .andThen(() -> m_robotDrive.stopModules())
    .andThen(new ParallelDeadlineGroup(new SequentialCommandGroup(secondCommand, new ShooterCmd(ShooterSubsystem, ShooterConstants.subwooferTopVelocity, ShooterConstants.subwooferBottomVelocity)), new conveyorClearCmd(ConveyorSubsystem, ConveyorConstants.kConveyorVelocity)))
